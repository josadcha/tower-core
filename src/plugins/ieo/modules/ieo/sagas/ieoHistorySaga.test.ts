...
describe('IEO History Fetch', () => {
    ...
    describe('Execute IEO history data', () => {
        ...
        const mockIEOOrderFetch = () => {
            mockAxios.onGet('/admin/ieo/orders?page=1&limit=25').reply(200, fakePayload, fakeHeaders);
        };

        const expectedActionsSuccess = [
            historyIEOFetch(fakeFetchPayloadFirstPage),
            historyIEOData(fakeSuccessPayloadFirstPage),
        ];
        ...
        it('should fetch IEO history in success flow', async () => {
            ...
            store.dispatch(historyIEOFetch(fakeFetchPayloadFirstPage));
